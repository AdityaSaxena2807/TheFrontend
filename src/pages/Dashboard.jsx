      ...
      {videos.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {videos.map((video) => (
            <div
              key={video._id}
              className="flex flex-col sm:flex-row sm:items-center gap-4 bg-surface rounded-md p-3 hover:bg-surface-elevated transition-colors duration-hover"
            >
              <Link
                to={`/watch/${video._id}`}
                className="shrink-0 w-full sm:w-40 aspect-video rounded-md overflow-hidden bg-bg"
              >
                <img
                  src={
